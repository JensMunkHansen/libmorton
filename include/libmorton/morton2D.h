...
inline morton m2D_e_sLUT(const coord x, const coord y)
{
  ...
  for (unsigned int i = sizeof(coord); i > 0; --i)
  {
    ...
    answer = answer << 16 | Morton2D_encode_y_256[(y >> shift) & EIGHTBITMASK] |
...
inline morton m2D_e_LUT(const coord x, const coord y)
{
  ...
  {
    unsigned int shift = (i - 1) * 8;
    answer = answer << 16 | (Morton2D_encode_x_256[(y >> shift) & EIGHTBITMASK] << morton(1)) |
      (Morton2D_encode_x_256[(x >> shift) & EIGHTBITMASK]);
  }
  return answer;
}
...
static uint_fast32_t magicbit2D_masks32[6] = { 0xFFFFFFFF, 0x0000FFFF, 0x00FF00FF, 0x0F0F0F0F,
  0x33333333, 0x55555555 };
static uint_fast64_t magicbit2D_masks64[6] = { 0x00000000FFFFFFFF, 0x0000FFFF0000FFFF,
  0x00FF00FF00FF00FF, 0x0F0F0F0F0F0F0F0F, 0x3333333333333333, 0x5555555555555555 };

// HELPER METHOD for Magic bits encoding - split by 2
template <typename morton, typename coord>
inline morton morton2D_SplitBy2Bits(const coord a)
{
  const morton* masks = (sizeof(morton) <= 4) ? reinterpret_cast<const morton*>(magicbit2D_masks32)
                                              : reinterpret_cast<const morton*>(magicbit2D_masks64);
  morton x = a;
  if (sizeof(morton) > 4)
  {
...
// ENCODE 2D 32-bit morton code - alternative version by JarkkoPFC -
// https://gist.github.com/JarkkoPFC/0e4e599320b0cc7ea92df45fb416d79a This uses the same technique
// as the magicbits method, but uses the upper part of a 64-bit type to split the y coordinate, the
// lower part to split the x coordinate, then merges them back together.
inline uint_fast32_t m2D_e_magicbits_combined(uint_fast16_t x, uint_fast16_t y)
{
  ...
  // hard cut off to 32 bits, because on some systems uint_fast32_t will be a 64-bit type, and we
  // don't want to retain split Y-version in the upper 32 bits.

#ifdef __SIZEOF_POINTER__
  // This macro specifies the size of a pointer in bytes
...
inline morton m2D_e_for(const coord x, const coord y)
{
  ...
  {
    // Can we always use 64-bit???
    morton mshifted = static_cast<morton>(0x1)
      << i; // Here we need to cast 0x1 to 64bits, otherwise there is a bug when morton code is
            // larger than 32 bits
...
inline morton m2D_e_for_ET(const coord x, const coord y)
{
  ...
  for (unsigned int i = 0; i < checkbits; ++i)
  {
    morton m_shifted = static_cast<morton>(0x1)
      << i; // Here we need to cast 0x1 to 64bits, otherwise there is a bug when morton code is
            // larger than 32 bits
    unsigned int shift = i;
    answer |= ((x & m_shifted) << shift) | ((y & m_shifted) << (shift + 1));
  }
  return answer;
}
...
template <typename morton, typename coord>
inline coord morton2D_DecodeCoord_LUT256(
  const morton m, const uint_fast8_t* LUT, const unsigned int startshift)
{
  morton a = 0;
...
{
  morton* masks = (sizeof(morton) <= 4) ? reinterpret_cast<morton*>(magicbit2D_masks32)
                                        : reinterpret_cast<morton*>(magicbit2D_masks64);
  morton x = m & masks[5];
  x = (x ^ (x >> 1)) & masks[4];
  x = (x ^ (x >> 2)) & masks[3];
  x = (x ^ (x >> 4)) & masks[2];
...
inline void m2D_d_for_ET(const morton m, coord& x, coord& y)
{
  ...
  unsigned int checkbits =
    static_cast<unsigned int>(std::min(defaultbits, firstbit_location / 2.0f));
  for (unsigned int i = 0; i <= checkbits; ++i)
  {
    morton selector = 1;
    unsigned int shift_selector = 2 * i;
