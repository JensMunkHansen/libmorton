...
struct encode_f_2D_wrapper
{
  ...
  encode_f_2D_wrapper(const string& description, morton (*encode)(coord, coord))
...
struct decode_f_2D_wrapper
{
  ...
  decode_f_2D_wrapper(const string& description, void (*decode)(morton, coord&, coord&))
    : description(description)
    , decode(decode)
  {
  }
...
struct encode_f_3D_wrapper
{
  ...
  encode_f_3D_wrapper(const string& description, morton (*encode)(coord, coord, coord))
    : description(description)
    , encode(encode)
  {
  }
...
struct decode_f_3D_wrapper
{
  ...
  decode_f_3D_wrapper(const string& description, void (*decode)(morton, coord&, coord&, coord&))
    : description(description)
    , decode(decode)
  {
  }
...
uint64_t split_by_n(uint64_t input, size_t bitsRemaining)
{
  return (bitsRemaining == 0)
    ? input
    : (split_by_n<FieldCount>(input >> 1, bitsRemaining - 1) << FieldCount) | (input & (uint64_t)1);
}
...
uint64_t join_by_n(uint64_t input, size_t bitsRemaining)
{
  return (bitsRemaining == 0)
    ? 0
    : (join_by_n<FieldCount>(input >> FieldCount, bitsRemaining - 1) << 1) | (input & (uint64_t)1);
}
...
uint64_t control_encode_impl(std::size_t bitCount, const std::valarray<uint64_t>& fields)
{
  ...
  return (control_encode_impl<FieldCount>(bitCount, fields[std::slice(1, fields.size() - 1, 1)])
           << 1) |
    split_by_n<FieldCount>(fields[std::slice(0, 1, 1)][0], bitCount);
}

/**
 * Runtime Morton encoding using 64-bit fields. Only the first floor(64 / |fields|) bits will be
 * encoded from each field.
 *
...
{
  return control_encode_impl<sizeof...(fields)>(
    std::numeric_limits<uint64_t>::digits / sizeof...(fields), { fields... });
}

template <std::size_t FieldCount>
void control_decode_impl(uint64_t encoding, std::size_t bitCount,
  const std::valarray<std::reference_wrapper<uint64_t>>& fields)
{
  if (fields.size() == 1)
  {
  ...
  fields[std::slice(0, 1, 1)][0].get() = join_by_n<FieldCount>(encoding, bitCount);
  control_decode_impl<FieldCount>(
    encoding >> 1, bitCount, fields[std::slice(1, fields.size() - 1, 1)]);
}
...
void control_decode(uint64_t encoding, T&... fields)
{
  ...
  return control_decode_impl<sizeof...(fields)>(
    encoding, std::numeric_limits<uint64_t>::digits / sizeof...(fields), list);
}
...
template <typename morton, typename coord>
void printIncorrectDecoding2D(
  string method_tested, morton m, coord x, coord y, coord correct_x, coord correct_y)
{
  std::cout << "\n    Incorrect decoding of " << getBitString<morton>(m) << " in method "
            << method_tested.c_str() << ": (" << getBitString<coord>(x) << ", "
            << getBitString<coord>(y) << ") != (" << getBitString<coord>(correct_x) << ", "
            << getBitString<coord>(correct_y) << ")\n";
}

template <typename morton, typename coord>
void printIncorrectDecoding3D(string method_tested, morton m, coord x, coord y, coord z,
  coord correct_x, coord correct_y, coord correct_z)
{
  std::cout << "\n    Incorrect decoding of " << getBitString<morton>(m) << " in method "
            << method_tested.c_str() << ": (" << getBitString<coord>(x) << ", "
            << getBitString<coord>(y) << ", " << getBitString<coord>(z) << ") != ("
