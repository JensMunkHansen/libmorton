...
static bool check2D_EncodeFunction(const encode_f_2D_wrapper<morton, coord>& function)
{
  ...
  static_assert(
    bits <= std::numeric_limits<uint64_t>::digits, "Control encoder cannot support > 64 bits.");
  static_assert(fieldbits >= 4, "At least 4 bits from each field must fit into 'morton'");
  static_assert(
    std::numeric_limits<morton>::digits >= 2 * fieldbits, "'morton' must support encoding width");
  static_assert(
    std::numeric_limits<coord>::digits >= fieldbits, "'coord' must support field width");

  bool everything_okay = true;
  morton computed_code;
  uint64_t correct_code;

  // For every set of 4 contiguous bits, test all possible values (0-15), with all other bits
  // cleared
    ...
    for (coord i = 0; i < 16; i++)
    {
      for (coord j = 0; j < 16; j++)
      {
        ...
        if (computed_code != (morton)correct_code)
        {
          ...
          std::cout << "\n    Incorrect encoding of (" << x << ", " << y << ") in method "
                    << function.description.c_str() << ": " << computed_code
                    << " != " << (morton)correct_code << "\n";
        }
      }
    }
...
static bool check2D_DecodeFunction(const decode_f_2D_wrapper<morton, coord>& function)
{
  ...
  static_assert(
    bits <= std::numeric_limits<uint64_t>::digits, "Control decoder cannot support > 64 bits.");
  static_assert(encodingbits >= 8, "'morton' must support at least 8 bit encodings");
  static_assert(
    std::numeric_limits<morton>::digits >= encodingbits, "'morton' must support encoding width");
  static_assert(
    std::numeric_limits<coord>::digits >= encodingbits / 2, "'coord' must support field width");
  ...
  for (size_t offset = 0; offset <= encodingbits - 8; offset++)
  {
    for (morton i = 0; i < 256; i++)
    {
      ...
      {
        printIncorrectDecoding2D<morton, coord>(
          function.description, encoding, x, y, (coord)xctrl, (coord)yctrl);
        everything_okay = false;
      }
    }
  }
  ...
  if (encodingbits >= 32)
  {
    ...
    {
      printIncorrectDecoding2D<morton, coord>(
        function.description, (morton)0xffffffff, x, y, 0xffff, 0xffff);
      everything_okay = false;
    }
  }
  ...
  if (encodingbits >= 64)
  {
    ...
    {
      printIncorrectDecoding2D<morton, coord>(
        function.description, (morton)0xffffffffffffffff, x, y, 0xffffffff, 0xffffffff);
      everything_okay = false;
    }
  }
  return everything_okay;
...
template <typename morton, typename coord, size_t bits>
inline bool check2D_Match(const encode_f_2D_wrapper<morton, coord>& encode,
  decode_f_2D_wrapper<morton, coord>& decode, unsigned int times)
{
  ...
  for (unsigned int i = 0; i < RAND_POOL_SIZE; ++i)
  {
    ...
    if ((x != x_result) || (y != y_result))
    {
      ...
      std::cout << bits << "-bit ";
      std::cout << "using methods encode " << encode.description << " and decode "
                << decode.description << "\n";
...
template <typename morton, typename coord, size_t bits>
inline bool check2D_EncodeDecodeMatch(std::vector<encode_f_2D_wrapper<morton, coord>> encoders,
  std::vector<decode_f_2D_wrapper<morton, coord>> decoders, unsigned int times)
{
  ...
  for (auto et = encoders.begin(); et != encoders.end(); et++)
  {
    for (auto dt = decoders.begin(); dt != decoders.end(); dt++)
    {
      ...
      if (!ok)
      {
        std::cout << dt->description << std::endl;
        exit(1);
      }
    }
  }
